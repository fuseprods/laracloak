...
@section('content')
    <div class="header">
        <h1>{{ __('Create Data Dashboard') }}</h1>
        <div class="user-menu">
    ...
    <div class="content-section wide">
        <form method="POST" action="{{ route('panel.dashboards.store') }}">
            @csrf
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 2rem;">
                <div class="form-group">
                    ...
                    <div style="display: flex; align-items: center;">
                        ...
                        <input type="text" id="slug" name="slug" value="{{ old('slug') }}" placeholder="monitoring"
                            style="border-radius: 0 0.5rem 0.5rem 0;" required>
                    </div>
                </div>

                <div class="form-group">
                    <label for="category-select">{{ __('Organization') }}</label>
                    ...
                    <div class="form-group">
                        ...
                        <select id="refresh_rate" name="refresh_rate">
                            @foreach([5 => '5s', 10 => '10s', 30 => '30s', 60 => '1m', 300 => '5m', 900 => '15m', 1800 => '30m', 3600 => '1h', 0 => 'Off'] as $val => $label)
                                <option value="{{ $val }}" {{ old('refresh_rate', 60) == $val ? 'selected' : '' }}>{{ $label }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div style="display: grid; grid-template-columns: 140px 1fr; gap: 1rem; margin-bottom: 1rem;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="url" id="destination_url" name="destination_url" value="{{ old('destination_url') }}"
                            placeholder="https://api.example.com/status" required>
                    </div>
                </div>
            ...
            <div class="form-group">
                ...
                <div id="json-mode" style="display: none;">
                    <textarea id="config" name="config" rows="12"
                        ...
                        placeholder='{ ... }'>{{ old('config', '{"widgets": []}') }}</textarea>
                </div>
            </div>
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function () {
                    ...
                    const originalClick = document.getElementById('btn-test-upstream').onclick;
            ...
            <div
                ...
                <label for="is_published" style="margin-bottom: 0; cursor: pointer; font-weight: 500;">
                    🚀 {{ __('Publish this dashboard immediately') }}
                    <small
            ...
            <div style="margin-top: 2rem; display: flex; justify-content: flex-end;">
                <button type="submit" class="btn btn-primary">{{ __('Create Dashboard') }}</button>
            </div>
        </form>
    </div>
