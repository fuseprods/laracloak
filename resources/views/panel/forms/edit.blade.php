...
@section('content')
    ...
    <div class="card" style="max-width: 900px; margin: 0 auto;">
        <form method="POST" action="{{ route('panel.forms.update', $form) }}">
            ...
            <div class="form-group">
                <label for="config">{{ __('Form Fields (JSON)') }}</label>
                <textarea id="config" name="config" rows="10"
                    style="font-family: monospace; background: #0f172a; color: #a5b4fc; padding: 1rem;">{{ old('config', json_encode($form->config, JSON_PRETTY_PRINT)) }}</textarea>
            </div>
